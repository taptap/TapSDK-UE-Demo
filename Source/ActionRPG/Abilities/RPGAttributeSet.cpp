...
void URPGAttributeSet::OnRep_Health(const FGameplayAttributeData& OldValue)
{
	...
	if (ARPGCharacter* Cha = Cast<ARPGCharacter>(GetOwningActor()))
	{
		Cha->HandleHealthChanged(GetHealth() - OldValue.GetCurrentValue(), FGameplayTagContainer(), nullptr);
	}
}
...
void URPGAttributeSet::OnRep_Mana(const FGameplayAttributeData& OldValue)
{
	...
	if (ARPGCharacter* Cha = Cast<ARPGCharacter>(GetOwningActor()))
	{
		Cha->HandleHealthChanged(GetMana() - OldValue.GetCurrentValue(), FGameplayTagContainer(), nullptr);
	}
}
...
void URPGAttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	...
	if (Data.EvaluatedData.Attribute == GetDamageAttribute())
	{
		...
		if (LocalDamageDone > 0)
		{
			...
			if (TargetCharacter)
			{
				...
				TargetCharacter->HandleHealthChanged(-LocalDamageDone, SourceTags, Causer);
			}
		}
	}
	else if (Data.EvaluatedData.Attribute == GetHealthAttribute())
	{
		...
		{
			// Call for all health changes
			TargetCharacter->HandleHealthChanged(DeltaValue, SourceTags, Causer);
		}
	}
